...
int pos1[MAX], pos2[MAX], par1[MAX], par2[MAX];
vector<int> vec1;
long long mask1[MAX];
...
void solve(){
	...
	for(int i = 0; i < min(a, b); i += 60){
		...
		for(auto x : vec1) for(auto y : adj1[x]) if(y != par1[x]) mask1[x] |= mask1[y];
		for(auto x : vec2) for(auto y : adj2[x]) if(y != par2[x]) mask2[x] |= mask2[y];
